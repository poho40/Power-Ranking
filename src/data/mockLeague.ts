...
function roster(team: number): Player[] {
  ...
  return slots.map((slot, i) => {const position=slot === "FLEX" ? "WR" : slot.replace("BE", i % 2 ? "RB" : "WR");return { id: `${team + 1}-${i + 1}`, name: `${["Avery","Jordan","Cameron","Riley","Morgan","Quinn","Hayden","Parker","Drew","Casey","Reese","Skyler"][i]} ${String.fromCharCode(65 + team)}.`, position, eligibleSlots:[position,...(["RB","WR","TE"].includes(position)?["FLEX"]:[])], proTeam: ["SF","BUF","DET","PHI","KC"][i % 5], slot, isStarter:slot!=="BE", fantasyPoints: Math.max(2, 22 - i * .9 + team * .35), projectedPoints: Math.max(2, 20 - i * .75 + (9-team) * .25),projectionSource:"weekly" as const }});
}
