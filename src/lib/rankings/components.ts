...
export function rosterScores(l:League){const raw=l.teams.map(t=>{const starters=t.roster.filter(p=>p.slot!=="BE"),bench=t.roster.filter(p=>p.slot==="BE");const avg=(ps:typeof t.roster)=>ps.length?ps.reduce((s,p)=>s+(p.weeklyProjectedPoints??p.seasonProjectedPoints??p.weeklyActualPoints??0),0)/ps.length:0;return .8*avg(starters)+.2*avg(bench)});return normalizedMap(l,raw);}
