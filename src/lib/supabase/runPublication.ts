import "server-only";
import {getLeague} from "@/lib/data/getLeague";import {recordPublishSuccess} from "@/lib/health/status";import {publishLatestCompletedWeek} from "./publication";import type {SnapshotStore} from "./types";
export async function runLatestPublication(store:SnapshotStore,loadLeague:typeof getLeague=getLeague){
  ...
  console.info("ESPN fetch and league identity validation succeeded.");const result=await publishLatestCompletedWeek(store,live.league);if(result.status==="published")await recordPublishSuccess().catch(()=>{console.warn("Ranking publication health update failed.")});console.info(`Ranking publication completed: ${result.status}.`);return{ok:true as const,httpStatus:200,body:result};
}
