...
export interface LineupAssignment { slot:string; slotIndex:number; player:Player; projectedPoints:number }
export interface OptimizedLineup { assignments:LineupAssignment[]; bench:Player[]; totalProjectedPoints:number }
...
export interface PreseasonPlayerContribution { playerId:string; name:string; position:string; projectedPoints:number; replacementLevel:number; valueOverReplacement:number; role:"starter"|"flex"|"depth" }
