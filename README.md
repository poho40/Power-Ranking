...
Weekly reports include reasons for optimism, concerns, and conditional upside/downside scenarios for every team, using only that article’s saved rankings snapshot. Week 1 explicitly treats scoring and recent form as a single week of evidence. Future news cron runs save these sections with the article. Older immutable articles (including Week 1) gain the sections when viewed, using their exact linked snapshot; their stored records, ranking values, and publication dates are preserved. The displayed editorial copy is refreshed to the current conversational format using the original snapshot. Deploy the updated generator, reader, and article component to enable this presentation. No database migration or republishing is needed.
